...
import React, {
    ...
    useState,
    useContext,
    ReactNode, // <<< Ensure comma is here if needed, and closing bracket below is correct
    useCallback,
    useMemo,
    useRef,
    useEffect,
...
export const TerminalProvider: React.FC<TerminalProviderProps> = ({ children }) => {
    ...
    const updateTerminalState = useCallback((terminalId: string, updates: Partial<OpenTerminal>) => {
    ...
    const createTerminal = useCallback(async (initialCols: number = 80, initialRows: number = 24) => {
        const newId = uuidv4();
        ...
        console.log(`TerminalContext: Creating new terminal tab (ID: ${newId}, Title: ${newTerminalTitle})`);
        // Update state first
        ...
        try {
            ...
            const result = await window.electronAPI.term_create({ id: newId, cols: colsToSend, rows: rowsToSend });
            if (result?.success) {
                console.log(`TerminalContext: Backend PTY creation request successful for ID ${newId}`);
                // Mark as ready now that the main process acknowledged creation
                updateTerminalState(newId, { isLoading: false, isReady: true, error: null });
            } else {
                console.error(`TerminalContext: Backend PTY creation failed for ID ${newId}:`, result?.error);
                updateTerminalState(newId, { isLoading: false, isReady: false, error: `Backend Error: ${result?.error || 'Unknown error'}` });
            }
        } catch (err) {
            const errorMsg = err instanceof Error ? err.message : String(err);
            console.error(`TerminalContext: IPC Error creating PTY for ID ${newId}:`, errorMsg);
            updateTerminalState(newId, { isLoading: false, isReady: false, error: `IPC Error: ${errorMsg}` });
        }

    }, [openTerminals.length, updateTerminalState]); // Dependency on length for title, and update helper

    // Function to close a terminal tab and kill backend PTY
    const closeTerminal = useCallback(async (terminalId: string) => {
        console.log(`TerminalContext: Requesting to close terminal ID: ${terminalId}`);
        // Find the index BEFORE filtering, needed for setting the next active tab
        const terminalToCloseIndex = openTerminals.findIndex(t => t.id === terminalId);
        const terminalToClose = openTerminals[terminalToCloseIndex];

        if (terminalToCloseIndex === -1 || !terminalToClose) {
            console.warn(`TerminalContext: Cannot close terminal ID ${terminalId}, not found.`);
            return;
        }

        // Remove from renderer state
        const remainingTerminals = openTerminals.filter(t => t.id !== terminalId);
        ...
        if (activeTerminalId === terminalId) {
            ...
            if (remainingTerminals.length > 0) {
                ...
                const nextIndex = Math.min(remainingTerminals.length - 1, Math.max(0, terminalToCloseIndex -1));
                nextActiveId = remainingTerminals[nextIndex]?.id || null;
            }
            console.log(`TerminalContext: Setting next active terminal to: ${nextActiveId}`);
        ...
        if ((terminalToClose.isReady || terminalToClose.isLoading) && terminalToClose.exitCode === undefined) {
            console.log(`TerminalContext: Sending kill request for backend PTY ID: ${terminalId}`);
            try {
                await window.electronAPI.term_kill(terminalId);
                console.log(`TerminalContext: Backend kill request acknowledged for ID: ${terminalId}`);
            } catch (err) {
                 console.error(`TerminalContext: IPC Error sending kill request for PTY ID ${terminalId}:`, err);
            }
        } else {
             console.log(`TerminalContext: No kill request needed for ID ${terminalId} (state: isLoading=${terminalToClose.isLoading}, isReady=${terminalToClose.isReady}, exitCode=${terminalToClose.exitCode}).`);
        }

    }, [openTerminals, activeTerminalId, setActiveTerminalId]); // Depends on state and the setter for active ID
    ...
    const setActiveTerminal = useCallback((terminalId: string | null) => {
        ...
        if (terminalId !== activeTerminalId) {
             console.log(`TerminalContext: Setting active terminal ID to: ${terminalId}`);
            setActiveTerminalId(terminalId);
        }
    }, [activeTerminalId]); // Depends only on activeTerminalId for comparison
    ...
    const writeToActiveTerminal = useCallback((data: string) => {
        ...
        if (activeTerm?.isReady && activeTerm.exitCode === undefined) {
             ...
             console.warn(`TerminalContext: Cannot write data, active terminal ${activeTerminalId} not ready or has exited.`);
        }
    }, [activeTerminalId, openTerminals]); // Depends on activeId and the list to find the term

    // Function to resize the *active* terminal's backend PTY
    const resizeActiveTerminal = useCallback((cols: number, rows: number) => {
        if (!activeTerminalId) {
         ...
         if (cols <= 0 || rows <= 0) {
            console.warn(`TerminalContext: Attempted to resize terminal ${activeTerminalId} with invalid dimensions: ${cols}x${rows}`);
            return;
         }
         ...
         if (activeTerm?.isReady && activeTerm.exitCode === undefined) {
            ...
            window.electronAPI.term_resize(activeTerminalId, { cols, rows });
             ...
             console.warn(`TerminalContext: Cannot resize, active terminal ${activeTerminalId} not ready or has exited.`);
         }
    ...
    useEffect(() => {
        ...
        console.log("TerminalContext: Attaching global IPC listeners (term_onData, term_onExit, term_onError)...");
        ...
        const handleData = (terminalId: string, data: string) => {
            ...
            const event = new CustomEvent< { data: string } >(`terminal-data-${terminalId}`, { detail: { data } });
        ...
        const handleExit = (terminalId: string, code?: number) => {
            console.log(`TerminalContext: Received exit signal for ID ${terminalId}, code: ${code}`);
            const exitCodeToStore = (typeof code === 'number') ? code : null; // Treat undefined exit as null
            updateTerminalState(terminalId, { isLoading: false, isReady: false, exitCode: exitCodeToStore });
            const event = new CustomEvent<{ code?: number | null }>(`terminal-exit-${terminalId}`, { detail: { code: exitCodeToStore } });
            window.dispatchEvent(event);
        };
        ...
        const handleError = (terminalId: string, errorMessage: string) => {
             console.error(`TerminalContext: Received error signal for ID ${terminalId}: ${errorMessage}`);
             // Mark as not loading/ready and store the error
             updateTerminalState(terminalId, { isLoading: false, isReady: false, error: errorMessage });
             const event = new CustomEvent<{ error: string }>(`terminal-error-${terminalId}`, { detail: { error: errorMessage } });
             window.dispatchEvent(event);
        };
    ...
    const contextValue = useMemo(() => ({
        ...
        openTerminals, activeTerminalId, createTerminal, closeTerminal,
        setActiveTerminal, writeToActiveTerminal, resizeActiveTerminal, getActiveTerminal
    ]);
